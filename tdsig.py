...
#đánh dấu bản quyền
nam="\033[1;31m[\033[1;37m=.=\033[1;31m] \033[1;37m=>  "
thanh = "\033[1;37m= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = ="
def banner():
 banner = f"""
            ...
            \033[1;35m██║ ╚████║██║  ██║██║ ╚═╝ ██║       ██║   ╚██████╔╝╚██████╔╝███████╗             
            \033[1;36m╚═╝  ╚═══╝╚═╝  ╚═╝╚═╝     ╚═╝       ╚═╝    ╚═════╝  ╚═════╝ ╚══════╝  
            \033[1;35m Youtube : \033[1;37mhttps://youtube.com/@NamTool1
            \033[1;35m Nhóm Zalo : \033[1;37mhttps://zalo.me/g/kfmgqm225
            \033[1;35m Facebook   : \033[1;37mhttps://facebook.com/nam.nhn131  
            \033[1;35m╰─────────────────────────────────────────────────────⋟─╯ 
            \033[1;37m    TOOL TRAO ĐỔI SUB INSTAGRAM
            \033[1;35m╰─────────────────────────────────────────────────────⋟─╯
"""
 for X in banner:
  sys.stdout.write(X)
...
os.system('espeak -a 300 "Welcome instagram sub exchange tool "')
banner()
token_tds = input(f'{nam}{trang}Vui Lòng Nhập Token TĐS{trang}:{vang} ')
check_log = requests.get('https://traodoisub.com/api/?fields=profile&access_token='+token_tds).json()
if 'success' in check_log:
    print(f'{xnhlucac}Đăng Nhập Thành Công!')
...
os.system("cls" if os.name == "nt" else "clear")
banner()
print(f'{nam}{trang}Tên Tài Khoản{trang}: {vang}{user}')
print(f'{nam}{trang}Xu Hiện Tại{trang}: {vang}{xucon}')
print(f'{nam}{trang}Xu Bị Trừ{trang}: {vang}{xudie}')
print(thanh)
print(f'{nam}{trang}Nhập Số {do}[{vang}1{do}] {trang}Để Chạy Nhiệm Vụ Tym')
print(f'{nam}{trang}Nhập Số {do}[{vang}2{do}] {trang}Để Chạy Nhiệm Vụ Follow')
print(f'{nam}{trang}Nhập Số {do}[{vang}3{do}] {trang}Để Chạy Nhiệm Vụ Tym Comment')
print(thanh)
chonjob = input(f'{nam}{trang}Nhập Nhiệm Vụ Bạn Muốn Chạy{trang}: {vang}')
delayjob = int(input(f'{nam}{trang}Nhập Delay Làm Nhiệm Vụ{trang}: {vang}'))
os.system("cls" if os.name == "nt" else "clear")
banner()
...
def delay_nam(o):
	while (o>1):
		o=o-1
		print(luc+' Tiếp Tục Sau <~> Nam~Tool '+do+'| '+vang+''+str(o)+do+' |','      ', end='\r')
		sleep(0.2)
		print(do+' Tiếp Tục Sau <~> Nam~Tool '+do+'| '+vang+''+str(o)+do+' |','      ', end='\r')
		sleep(0.1)
		print(vang+' Tiếp Tục Sau <~> Nam~Tool '+do+'| '+vang+''+str(o)+do+' |','      ', end='\r')
		sleep(0.2)
		print(hong+' Tiếp Tục Sau <~> Nam~Tool '+do+'| '+vang+''+str(o)+do+' |','      ', end='\r')
		sleep(0.1)
		print(xnhac+' Tiếp Tục Sau <~> Nam~Tool '+do+'| '+vang+''+str(o)+do+' |','      ', end='\r')
		sleep(0.2)
def loadjob(o):
	while (o>1):
		o=o-1
		print(luc+' Đang Tìm Nhiệm Vụ <~> Nam~Tool '+do+'| '+vang+''+str(o)+do+' |','      ', end='\r')
		sleep(0.2)
		print(do+' Đang Tìm Nhiệm Vụ <~> Nam~Tool '+do+'| '+vang+''+str(o)+do+' |','      ', end='\r')
		sleep(0.1)
		print(vang+' Đang Tìm Nhiệm Vụ <~> Nam~Tool '+do+'| '+vang+''+str(o)+do+' |','      ', end='\r')
		sleep(0.2)
		print(hong+' Đang Tìm Nhiệm Vụ <~> Nam~Tool '+do+'| '+vang+''+str(o)+do+' |','      ', end='\r')
		sleep(0.1)
		print(xnhac+' Đang Tìm Nhiệm Vụ <~> Nam~sTool '+do+'| '+vang+''+str(o)+do+' |','      ', end='\r')
		sleep(0.2)
...
try:
    ...
    pass
#info cấu hình
print(f'{nam_tool}{trang}USER CẤU HÌNH{trang}: {vang}{user_ch} {do}| {trang}ID CẤU HÌNH{trang}: {vang}{id_ig}')
print(thanh)
#chế độ tym
if '1' in chonjob:
    while True:
